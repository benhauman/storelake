...
SELECT * FROM hlsysum_query_centraladminorgunits(@volume, @agentid, @filter_text)
*/
CREATE FUNCTION [dbo].[hlsysum_query_centraladminorgunits]
(
	  @volume INT
	, @agentid INT
...
(
	...
	(
		...
		SELECT TOP(@volume) bu.orgunitid, bu.orgunitdefid, bu._fnd, 0 AS __cnt, bu._last, bu._lastdef, 0 AS __last, 0 AS __lastdef, 0 AS __scnd_last, 0 AS __third_last
		   FROM query_bottom_up AS bu
	       LEFT OUTER JOIN query_bottom_up AS bus
				ON bu.orgunitid = bus._last AND bu.orgunitdefid = bus._lastdef
			CROSS APPLY ( 
				SELECT 1 AS canread 
					FROM dbo.hlsyssec_query_agentobjectprmread(@agentid, bu.orgunitid, bu.orgunitdefid, 787) AS secrlst 
					WHERE secrlst.canread = 1 
			) AS sec
			WHERE bus.orgunitid IS NULL
