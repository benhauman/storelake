CREATE FUNCTION [dbo].[hlsyssec_query_agentglobalmsk] 
(
	@agentid INT,
	@globalid INT
)
RETURNS TABLE AS RETURN
	...
	(
		SELECT ag.agentid, globalid = gp.id, ag.groupid
	...
	JOIN [dbo].[hlsysglobalacl] AS gp 
	ON gp.groupid = ag.groupid AND ag.agentid = @agentid AND gp.id = @globalid
	--group by ag.agentid, gp.globalid
	)
	SELECT aprm.globalid
		,  CAST(0
			+ MAX(aprm.perm01)* 1
			+ MAX(aprm.perm02)*2
			+ MAX(aprm.perm03)*4
